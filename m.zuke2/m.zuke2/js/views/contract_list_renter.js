    ...
    contract.prototype = {
        ...
        delContract: (function() {
            var that,
                id,
                thatParent;
            $(document).on('click', '.contract .end', function(e) {
                e.preventDefault();
                var that = $(this);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        that.addClass('frozen').html("已结束");
                    },
                    error: function() {
                        contract.prototype.layerFun("提前结束失败，请刷新页面");
                    }
                });
            });
        })(),
        ...
        getTemplate: function(data) {
            return '<div class="my-box" data-id="' + data.id + '">' +
                ...
                '<p class="f12 mb4"><span class="c-999">租客：</span><span class="c-333">' + data.renter_name + '</span><span class="c-999 ml35">联系电话：</span><span class="c-333">' + data.renter_phone + '</span></p>' +
                '</div>' +
                '<div class="btns flex align-items-center">' +
                '<p class="f12 plr12"><span class="c-999">已支付：</span><span class="c-3fabfa">' + data.pay.month + '</span><span class="c-999">个月</span></p>' +
                '<div class="flex flex-right">' +
                '<a href="">付租</a>' +
                '<a class="end flex-right">提前结束</a>' +
                '</div>' +
                '</div>' +
                '</div>';
        }

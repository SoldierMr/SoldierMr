    ...
    yuyue.prototype = {
        ...
        cancelYuyue: (function() {
            var _this = yuyue,
                ...
                houseId,
                stop = true;
            $(document).on('click', '.cancel-yuyue', function(e) {
                e.preventDefault();
                that = $(this);
                ...
                if (stop) {
                    ...
                    $.ajax({
                        ...
                        success: function(json) {
                            ...
                            _this.prototype.layerFun("已取消预约");
                            stop = true; // 重置触发开关
                        },
                        error: function() {
                            _this.prototype.layerFun("取消失败，请刷新页面");
                        }
                    });
                }
            });
        ...
        getTemplate: function(data) {
            return '<div class="my-box" data-id="' + data.id + '">' +
                ...
                '<p class="f12"><span class="c-999">预约时间：</span><span class="c-333">' + data.time + '</span></p>' +
                '</div>' +
                '<div class="btns flex">' +
                '<a class="flex-right cancel-yuyue">取消预约</a>' +
                '</div>' +
                '</div>';
        }
    };
